...
-- valib kõik kirjed tabelist Person, mille väärtus veerus Name ei alga tähega "W", "A" või "S".
select * from Person where Name like '[^WAS]%'
--- valib kõik kirjed tabelist Person, kus veeru City väärtus on "Gotham" või "New York" ja veeru Age väärtus on suurem või võrdne 40.
...
-- Sorteerib kirjed selle täisarvulise väärtuse järgi
select * from Person order by cast(Age as int)
select * from Person order by Age

-- võtab kokku tabeli Person veeru Age väärtuse pärast nende teisendamist täisarvuliseks andmetüübiks.
...
--- protseduur tagastab nimed soo ja osakonna järgi
...
-- tagastab määratud sooga töötajate arvu EmployeeCount'i kaudu.
...
--- protseduur tagastab @total count kaudu töötajate tabelis olevate kirjete koguarvu.
...
--- tagastab töötaja nime antud identifikaatori (@Id) järgi. 
...
-- tagastab töötaja nime selle identifikaatori (6) järgi ja kuvab selle ekraanil.
declare @FirstName nvarchar(50)
execute spGetNameById1 6, @FirstName output
print 'Name of the employee = ' + @FirstName

-- tagastab määratud identifikaatoriga (@Id) töötaja nime.
...
-- kutsuda spGetNameById2-protseduuri töötaja ID 1 ja salvestada tulemus muutujasse @EmployeeName.
