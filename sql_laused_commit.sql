...
--- funktsioon lisab võõrvõtme piirangu veerusuhte jaoksi
...
--funktsioon lisab selles veerus piirväärtuse, mis on suurem kui 0 ja väiksem kui 150.
alter table Person
...
--Väljastab kõik kirjed tabelist, kustutab id-8 kirjed, väljastab kirjed ilma id-8.
select * from Person
go
...
-- Esimene päring teisendab vanuse veeru väärtused enne sorteerimist täisarvudeks ja teine päring lihtsalt sorteerib vanuse väärtused.
select * from Person order by cast(Age as int)
select * from Person order by Age

-- arvutab kõigi tabelis olevate kirjete vanuse summa.select sum(cast(Age as int)) from Person
...
--Nime "Tom", kesknime "Nick" ja perekonnanime "Jones" määramine ID-ga 1.
...
-- tagastab töötajate arvu soo genderi
...
--- arvutab kirjete koguarvu tabelis Employees
...
--- tagastab töötaja nime antud identifikaatori järgi.
...
-- tagastab töötaja nime antud id järgi
...
-- saada töötaja nimi ja kuvada see
